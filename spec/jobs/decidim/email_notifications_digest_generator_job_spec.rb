...
describe Decidim::EmailNotificationsDigestGeneratorJob, :anonymity do
  let!(:public_user) { create(:user, :published, notifications_sending_frequency: frequency) }
  let!(:private_user) { create(:user, notifications_sending_frequency: frequency) }
  let!(:anonymous_user) { create(:user, :anonymous, notifications_sending_frequency: frequency) }
  let(:frequency) { "daily" }
  ...
  it "updates digest_sent_at for both private and public users" do
    ...
    expect(anonymous_user.digest_sent_at).to be_nil

    job.perform(public_user.id, frequency, time:)
    job.perform(private_user.id, frequency, time:)
    job.perform(anonymous_user.id, frequency, time:)

    expect(public_user.reload.digest_sent_at).not_to be_nil
    expect(private_user.reload.digest_sent_at).not_to be_nil
    expect(anonymous_user.reload.digest_sent_at).not_to be_nil
  end
end
